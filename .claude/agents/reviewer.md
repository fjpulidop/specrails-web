...
- ESLint `@typescript-eslint/no-unused-vars` is OFF in this project — but unused imports still cause build issues
- TypeScript strict mode catches type errors that loose mode would allow
- Vite build may fail on imports that work in dev mode (e.g., missing file extensions for non-TS files)
- Test files import from `vitest` — ensure `vitest` is in devDependencies
...
- [ ] Dracula theme CSS custom properties used for all colors
- [ ] No hardcoded color values (hex, rgb, hsl literals)
- [ ] shadcn/ui components used where applicable
- [ ] No `any` types
- [ ] No console.log statements left in production code

### Test Quality
- [ ] New components have at least basic render tests
...
- Dracula theme CSS custom properties must be used — never introduce hardcoded colors
- shadcn/ui component patterns must be respected
- Do not remove or weaken TypeScript strict mode checks
- Do not add eslint-disable comments unless absolutely necessary and documented
