...
describe("FeaturesSection", () => {
  ...
  it("renders the section heading and eyebrow", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Built for shipping/i }),
    ).toBeInTheDocument();
  });
  ...
  it("clicking Hub tab shows hub features", async () => {
    ...
    expect(screen.getByText("Real-Time Rails")).toBeInTheDocument();
  ...
  it("clicking Together tab shows combined value cards", async () => {
    ...
    expect(
      screen.getByText("Core Implements, Hub Visualizes"),
    ).toBeInTheDocument();
    expect(screen.getByText("Core Ships PRs, Hub Tracks")).toBeInTheDocument();
  });
