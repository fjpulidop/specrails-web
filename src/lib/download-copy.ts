...
export const DOWNLOAD_COPY: Record<LanguageId, DownloadCopy> = {
  en: {
    ...
    included: "The installer includes everything Specrails needs, including Specrails Core, the engine that prepares your projects and runs implementations. You don’t install Core separately; you only need the AI provider CLI you want to use.",
  },
  es: {
    ...
    included: "El instalador incluye todo lo que necesita Specrails, también Specrails Core, el motor que prepara tus proyectos y ejecuta las implementaciones. No instalas Core por separado: solo necesitas la CLI del proveedor de IA que quieras usar.",
  },
  fr: {
    ...
    included: "L’installateur contient tout ce dont Specrails a besoin, y compris Specrails Core, le moteur qui prépare vos projets et exécute les implémentations. Core ne s’installe pas séparément : il vous suffit de la CLI du fournisseur d’IA de votre choix.",
  },
  de: {
    ...
    included: "Das Installationspaket enthält alles, was Specrails braucht, auch Specrails Core, die Engine, die Ihre Projekte vorbereitet und Implementierungen ausführt. Core wird nicht separat installiert; Sie benötigen nur die CLI des KI-Anbieters Ihrer Wahl.",
  },
  pt: {
    ...
    included: "O instalador inclui tudo de que o Specrails precisa, inclusive o Specrails Core, o motor que prepara seus projetos e executa as implementações. Você não instala o Core separadamente; só precisa da CLI do provedor de IA que quiser usar.",
  },
  it: {
    ...
    included: "Il programma di installazione contiene tutto ciò che serve a Specrails, incluso Specrails Core, il motore che prepara i progetti ed esegue le implementazioni. Core non si installa a parte: ti basta la CLI del provider di IA che vuoi usare.",
  },
  zh: {
    ...
    included: "安装程序已包含 Specrails 所需的一切，包括 Specrails Core——负责准备项目并执行实现的引擎。无需单独安装 Core，您只需准备要使用的 AI 提供商 CLI。",
  },
  ja: {
    ...
    included: "インストーラーには Specrails に必要なものがすべて含まれています。プロジェクトを準備して実装を実行するエンジン、Specrails Core も同梱です。Core を別途インストールする必要はなく、使用する AI プロバイダーの CLI を用意するだけです。",
  },
};
