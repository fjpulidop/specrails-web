...
Agents asked to connect Claude, Codex, Kimi or Gemini to Specrails MCP should start at [the dedicated runbook](src/content/for-agents/mcp.md). The built site exposes `/llms.txt`, `/for-agents/index.html`, `/for-agents/mcp.md` as static files, without JavaScript. The docs index and MCP article link to this section.
