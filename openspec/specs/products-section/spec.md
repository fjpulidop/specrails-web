...
The system SHALL render a ProductsSection component between ProblemSection and AgentsSection on the home page. The section SHALL display two side-by-side cards: specrails-core ("The Engine") on the left and specrails-desktop ("The Control Center") on the right. Each card SHALL include the product name, a tagline, an icon, and 4-5 key capability bullet points. Each card SHALL have an individual CTA button. A visual connector element between the cards SHALL indicate the products work together.
...
- **AND** the right card shows specrails-desktop with LayoutDashboard icon, tagline "The Control Center", and capabilities: Multi-Project Dashboard, Real-time Pipeline Visualization, Ticket Management (3 views), Analytics & Cost Tracking, Streaming Logs

#### Scenario: Mobile layout stacks cards vertically
- **WHEN** the user views the home page on a viewport < 1024px
- **THEN** the two product cards stack vertically (Core on top, Hub below)

#### Scenario: CTA buttons link to installation
- **WHEN** the user clicks the Core CTA button
- **THEN** the page scrolls to the hero install terminal section
- **WHEN** the user clicks the Hub CTA button
- **THEN** the page scrolls to the hub-showcase section
